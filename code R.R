...
png("Histogram2.png", width = 1200, height = 900, res = 120)
...
cor_matrix <- cor(water_data[,1:9], use = "complete.obs")
...
model<-glm(Potability~ph+Hardness+Solids+Chloramines+
...
set.seed(8)
modelrf<-randomForest(Potability~.,data=train_data,
                         ntree=500,   
                         mtry=3,  
                         importance=TRUE) 
